    ...
    <div className="modal-backdrop" aria-hidden="false">
      <div ref={panelRef} className="card w-full max-w-3xl overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-200 flex items-center justify-between gap-3">
          <div>
            <div className="text-xs font-extrabold text-slate-500">Saved</div>
            <div className="text-lg font-extrabold">Bài viết đã lưu</div>
          </div>
        ...
        <div className="p-6">
          {items.length === 0 ? (
            <div className="text-center text-slate-600 py-10">
              <div className="text-3xl"><i className="fa-solid fa-inbox" /></div>
              <div className="mt-2 font-extrabold">Chưa có bài lưu</div>
              <div className="text-sm mt-1">Nhấn “Lưu” ở một bài viết để thấy nó xuất hiện tại đây.</div>
            </div>
          ) : (
                ...
                <div key={p.id} className="rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-200">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      ...
                      <div className="text-sm text-slate-600 mt-1">{tagLabel(p.tag)} • {p.date}</div>
                    </div>
                    <button className="btn h-10 w-10 p-0" onClick={() => props.onToggleSave(p.id)} title="Bỏ lưu">
                      <i className="fa-solid fa-bookmark text-indigo-600" />
                    </button>
                  </div>

                  <div className="mt-3 flex gap-2">
                    <button className="btn btn-primary flex-1" onClick={() => props.onRead(p.id)}>
                      <i className="fa-solid fa-book-open" /> Đọc
                    </button>
                    <button
