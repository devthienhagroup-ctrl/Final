...
export function BookingForm(props: any) {
  ...
  const create = () => {
    if (!draft.name.trim()) return onToast(data.errorMessage, data.validationToasts.missingName, "error");
    if (!isValidPhone(draft.phone.trim())) return onToast(data.errorMessage, data.validationToasts.invalidPhone, "error");
    if (!draft.date) return onToast(data.errorMessage, data.validationToasts.missingDate, "error");
    if (!svc) return onToast(data.errorMessage, data.validationToasts.missingService, "error");
    if (!branchPick) return onToast(data.errorMessage, data.validationToasts.missingBranch, "error");
    if (!selectedSlot) return onToast(data.errorMessage, data.validationToasts.missingTime, "error");
