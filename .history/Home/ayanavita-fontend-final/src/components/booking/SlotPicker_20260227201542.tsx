  ...
  return (
    <aside className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex items-start justify-between">
        <div>
          <div className="text-xs font-extrabold text-slate-500">{data.headingKicker}</div>
          <div className="text-xl font-extrabold">{data.headingTitle}</div>
          ...
          return (
            ...
            >
              <div className="text-sm font-extrabold">{s.t}</div>
                    <span className="text-slate-500">{disabled ? data.unavailableMark : data.availableMark}</span>
            </button>
          );
        })}
      </div>
