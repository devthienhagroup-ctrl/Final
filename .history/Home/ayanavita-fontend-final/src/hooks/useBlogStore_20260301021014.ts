...
export function useBlogStore() {
  ...
  const trackView = useCallback(async (id: string) => {
    ...
    try {
      const res = await http.get<PublicBlogItem>(`/blogs/public/${numericId}`);
      const nextViews = res.data?.views;
      if (typeof nextViews !== "number") return;

      setPosts((prev) => prev.map((post) => (post.id === id ? { ...post, views: nextViews } : post)));
    } catch {
      // tracking view thất bại thì bỏ qua để không ảnh hưởng UX mở modal đọc bài
    }
  }, []);
