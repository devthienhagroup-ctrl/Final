...
const demoMyOrders: MyOrder[] = [
  {
    code: "ORD-250310-001",
    status: "paid",
    createdAt: "2026-02-10",
    branch: "CN Quận 1",
    payment: { method: "Chuyển khoản", paidAt: "2026-02-10", ref: "TX-382911" },
    pricing: { subtotal: 1250000, shipping: 30000, discount: 100000, total: 1180000 },
    note: "Giao giờ hành chính.",
    items: [
      { name: "Tinh dầu thư giãn Lavender", sku: "AYA-OIL-LAV-10", qty: 1, price: 350000, image: "https://images.unsplash.com/photo-1615485290382-441e4d049cb5?q=80&w=400&auto=format&fit=crop" },
      { name: "Nến thơm thiền định", sku: "AYA-CAN-MED-02", qty: 2, price: 220000, image: "https://images.unsplash.com/photo-1603006905003-be475563bc59?q=80&w=400&auto=format&fit=crop" },
      { name: "Bộ trà thảo mộc ban đêm", sku: "AYA-TEA-SLP-06", qty: 1, price: 460000, image: "https://images.unsplash.com/photo-1544787219-7f47ccb76574?q=80&w=400&auto=format&fit=crop" },
    ],
    shippingInfo: {
      receiverName: "Nguyễn Minh Anh",
      phone: "0901 234 567",
      addressLine: "12 Nguyễn Huệ, P.Bến Nghé",
      district: "Quận 1",
      city: "TP.HCM",
      carrier: "GHN Express",
      trackingCode: "TRK-783291",
      expectedDelivery: "2026-02-13",
      note: "Đang đóng gói & sẵn sàng bàn giao.",
    },
  },
  {
    code: "ORD-250226-002",
    ...
    branch: "CN Thủ Đức",
    payment: { method: "COD", paidAt: null, ref: null },
    pricing: { subtotal: 650000, shipping: 25000, discount: 0, total: 675000 },
    note: "Gọi trước khi giao.",
    items: [
      { name: "Serum phục hồi da", sku: "AYA-SER-REC-30", qty: 1, price: 650000, image: "https://images.unsplash.com/photo-1556228720-195a672e8a03?q=80&w=400&auto=format&fit=crop" },
    ],
    shippingInfo: {
      receiverName: "Trần Ngọc Lan",
      phone: "0912 345 678",
      addressLine: "88 Võ Văn Ngân, P.Linh Chiểu",
      district: "Thủ Đức",
      city: "TP.HCM",
      carrier: "J&T Express",
      trackingCode: null,
      expectedDelivery: "2026-03-01",
      note: "Chưa bàn giao cho đơn vị vận chuyển.",
    },
  },
  {
    code: "ORD-250215-003",
    ...
    payment: { method: "Ví điện tử", paidAt: null, ref: null },
    pricing: { subtotal: 420000, shipping: 20000, discount: 20000, total: 420000 },
    note: "Người dùng yêu cầu hủy do đổi địa chỉ.",
    items: [
      { name: "Combo chăm sóc tóc", sku: "AYA-HAIR-COM-01", qty: 1, price: 420000, image: "https://images.unsplash.com/photo-1522335789203-aabd1fc54bc9?q=80&w=400&auto=format&fit=crop" },
    ],
    shippingInfo: {
      receiverName: "Lê Thanh Hương",
      phone: "0938 888 999",
      addressLine: "5 Nguyễn Gia Trí, P.25",
      district: "Bình Thạnh",
      city: "TP.HCM",
      carrier: "AhaMove",
      trackingCode: null,
      expectedDelivery: "2026-02-18",
      note: "Đơn đã hủy trước khi bàn giao vận chuyển.",
    },
  },
];
...
export default function AccountCenter() {
  ...
  const filteredOrders = useMemo(() => {
    ...
    return demoMyOrders.filter((order) => {
  ...
  }, [orderKeyword, orderStatusFilter]);
                  ...
                  {filteredOrders.length === 0 ? (
                    ...
                    <div className="space-y-3">
                      {filteredOrders.map((order) => (
                        <article key={order.code} className="rounded-2xl border border-slate-200 bg-white p-4">
                          <div className="flex flex-wrap items-center justify-between gap-3">
                            <div>
                              <h3 className="text-base font-extrabold text-slate-900">{order.code}</h3>
                              <p className="mt-1 text-sm text-slate-600">
                                {formatDate(order.createdAt)} • {order.payment.method} • {formatMoney(order.pricing.total)}
                              </p>
                            </div>
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <button
                                ...
                                className="inline-flex items-center gap-2 rounded-xl border border-indigo-200 bg-indigo-50 px-3 py-2 text-xs font-extrabold text-indigo-700 hover:bg-indigo-100"
                              >
                  ...
                  {selectedOrder && (
                    <div className="fixed inset-0 z-[120] flex items-center justify-center bg-slate-900/50 px-4 py-6" onClick={() => setSelectedOrder(null)}>
                      <div className="max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-3xl bg-white p-5 shadow-2xl md:p-6" onClick={(e) => e.stopPropagation()}>
                        <div className="mb-4 flex items-start justify-between gap-3">
                          <div>
                            <p className="text-xs font-bold uppercase tracking-wide text-slate-500">{cms.myOrders.detailTitle}</p>
                        ...
                        <div className="grid gap-2 text-sm text-slate-700 md:grid-cols-2">
                          <p><span className="font-semibold">{cms.myOrders.fields.createdAt}:</span> {formatDate(selectedOrder.createdAt)}</p>
                          <p><span className="font-semibold">{cms.myOrders.fields.branch}:</span> {selectedOrder.branch}</p>
                          <p><span className="font-semibold">{cms.myOrders.fields.paymentMethod}:</span> {selectedOrder.payment.method}</p>
                          <p><span className="font-semibold">{cms.myOrders.fields.paymentRef}:</span> {selectedOrder.payment.ref || "—"}</p>
                          <p><span className="font-semibold">{cms.myOrders.fields.paidAt}:</span> {formatDate(selectedOrder.payment.paidAt)}</p>
                          <p><span className="font-semibold">{cms.myOrders.fields.receiver}:</span> {selectedOrder.shippingInfo.receiverName}</p>
                          <p><span className="font-semibold">{cms.myOrders.fields.phone}:</span> {selectedOrder.shippingInfo.phone}</p>
                          <p><span className="font-semibold">{cms.myOrders.fields.carrier}:</span> {selectedOrder.shippingInfo.carrier}</p>
                          <p><span className="font-semibold">{cms.myOrders.fields.trackingCode}:</span> {selectedOrder.shippingInfo.trackingCode || "—"}</p>
                          <p><span className="font-semibold">{cms.myOrders.fields.expectedDelivery}:</span> {formatDate(selectedOrder.shippingInfo.expectedDelivery)}</p>
                          <p className="md:col-span-2"><span className="font-semibold">{cms.myOrders.fields.address}:</span> {`${selectedOrder.shippingInfo.addressLine}, ${selectedOrder.shippingInfo.district}, ${selectedOrder.shippingInfo.city}`}</p>
                          <p className="md:col-span-2"><span className="font-semibold">{cms.myOrders.fields.orderNote}:</span> {selectedOrder.note}</p>
                          <p className="md:col-span-2"><span className="font-semibold">{cms.myOrders.fields.shippingNote}:</span> {selectedOrder.shippingInfo.note}</p>
                        </div>

                        <div className="mt-5 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                          <h4 className="mb-3 text-sm font-extrabold text-slate-900">{cms.myOrders.productsTitle}</h4>
                          <div className="space-y-3">
                            {selectedOrder.items.map((item) => (
                              <div key={item.sku} className="grid items-center gap-3 rounded-2xl border border-slate-200 bg-white p-3 md:grid-cols-[64px_1fr_auto]">
                                <img src={item.image} alt={item.name} className="h-16 w-16 rounded-xl object-cover" />
                                <div>
                                  <p className="font-bold text-slate-900">{item.name}</p>
                                  <p className="text-xs text-slate-500">{cms.myOrders.fields.sku}: {item.sku}</p>
                                </div>
                                <div className="text-right text-sm text-slate-700">
                                  <p>{cms.myOrders.fields.quantity}: x{item.qty}</p>
                        ...
                        <div className="mt-4 grid gap-2 rounded-2xl border border-slate-200 bg-white p-3 text-sm text-slate-700 md:grid-cols-2">
                          <p><span className="font-semibold">{cms.myOrders.fields.subtotal}:</span> {formatMoney(selectedOrder.pricing.subtotal)}</p>
                          <p><span className="font-semibold">{cms.myOrders.fields.shipping}:</span> {formatMoney(selectedOrder.pricing.shipping)}</p>
                          <p><span className="font-semibold">{cms.myOrders.fields.discount}:</span> -{formatMoney(selectedOrder.pricing.discount)}</p>
                          <p className="font-extrabold text-slate-900"><span>{cms.myOrders.fields.total}:</span> {formatMoney(selectedOrder.pricing.total)}</p>
                        </div>
                      </div>
                    </div>
                  )}
