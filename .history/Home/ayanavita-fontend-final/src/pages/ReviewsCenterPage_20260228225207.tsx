...
export default function ReviewsCenterPage() {
  ...
  async function mergeLocalHelpfulWhenLogin() {
    if (!isLoggedIn) return;
    const localMap = loadVoteMap();
    const localIds = getActiveHelpfulReviewIds(localMap);
    if (localIds.length) {
      await http.post("/reviews/helpful/merge-local", { reviewIds: localIds });
      clearVoteMap();
    }
  }
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        if (isLoggedIn) {
          await mergeLocalHelpfulWhenLogin();
...
setModalOpen(false);
  ...
  function openHelpfulModal() {
    if (!isLoggedIn) {
      alert("Vui lòng đăng nhập để xem lịch sử đã đánh dấu hữu ích.");
      return;
    }
