...
type PublicReviewApi = {
  ...
  product?: { id?: string | number; translations?: Array<{ name?: string; slug?: string }> } | null;
  images?: Array<{ imageUrl?: string }>;
  helpfulCount?: number;
};

type HelpfulHistoryResponse = {
  page: number;
  pageSize: number;
  total: number;
  items: PublicReviewApi[];
};
...
function mapPublicReview(row: PublicReviewApi): Review {
  const firstProductTranslation = row.product?.translations?.[0];
  const category = row.type === "PRODUCT" ? "product" : "service";
  const productSlug = firstProductTranslation?.slug;
  const productId = row.product?.id;
  const serviceId = row.service?.id;
  const itemPath =
    category === "product"
      ? productSlug || productId
        ? `/products/${productSlug || productId}`
        : undefined
      : serviceId
      ? `/services/${serviceId}`
      : undefined;
  ...
  return {
    ...
    category,
    item: row.service?.name || firstProductTranslation?.name || "",
    itemPath,
    ...
    helpful: Number(row.helpfulCount || 0),
...
export default function ReviewsCenterPage() {
  ...
  const [helpfulModalOpen, setHelpfulModalOpen] = useState(false);
  const [helpfulHistory, setHelpfulHistory] = useState<Review[]>([]);
  const [helpfulPage, setHelpfulPage] = useState(1);
  const [helpfulTotal, setHelpfulTotal] = useState(0);
  const helpfulPageSize = 5;
  ...
  const [reviewableProducts, setReviewableProducts] = useState<ProductOption[]>([]);
  ...
  async function fetchMyHelpfulVotes() {
    if (!isLoggedIn) return;
    const res = await http.get<HelpfulHistoryResponse>("/reviews/my-helpful-reviews?page=1&pageSize=100");
    const nextMap: Record<string, boolean> = {};
    (res.data?.items || []).forEach((row) => {
      nextMap[String(row.id)] = true;
    });
    setVoteMap(nextMap);
  }
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        if (isLoggedIn) {
          await fetchMyHelpfulVotes();
        }
      } catch (e) {
        console.error("Failed to sync helpful votes", e);
        setVoteMap({});
      }
  ...
  useEffect(() => {
    (async () => {
      try {
        const [bRes, sRes] = await Promise.all([http.get("/reviews/branches"), http.get("/reviews/services")]);
        setBranches((bRes.data || []).map((b: any) => ({ id: Number(b.id), name: b.name })));
        setServices((sRes.data || []).map((x: any) => ({ id: Number(x.id), name: x.name })));
      } catch (e) {
        console.error("Failed to load review options", e);
      }
    })();
  }, []);
  ...
  useEffect(() => {
    if (!modalOpen) return;
    if (fCat !== "product" || !isLoggedIn) {
      setReviewableProducts([]);
      return;
    }
    (async () => {
      try {
        const res = await http.get("/reviews/my-product-reviewables");
        setReviewableProducts(res.data || []);
      } catch (e) {
        console.error("Failed to load purchased products", e);
      }
    })();
  }, [modalOpen, fCat, isLoggedIn]);

  useEffect(() => {
    // lock scroll when modal/drawer open
    document.body.style.overflow = modalOpen || drawerOpen || helpfulModalOpen ? "hidden" : "";
    return () => {
      document.body.style.overflow = "";
    };
  }, [modalOpen, drawerOpen, helpfulModalOpen]);
  ...
  async function toggleHelpful(id: string) {
    ...
    try {
      const res = await http.post(`/reviews/${id}/helpful/toggle`);
      const next = [...reviews];
      next[idx] = {
        ...next[idx],
        helpful: Number(res.data?.helpfulCount || 0),
      };
      setReviews(next);
      setVoteMap((prev) => ({ ...prev, [id]: !!res.data?.active }));
    } catch (e) {
  ...
  async function submitReview() {
    ...
    if (!name || !text || !branchId) {
      alert("Vui lòng nhập tên, nội dung và chọn chi nhánh.");
      return;
    }
    ...
    try {
      ...
      const categoryType = fCat === "product" ? "PRODUCT" : "SERVICE";
      payload.append("type", categoryType);
      payload.append("branchId", String(branchId));
      ...
      if (fCat === "service") {
        ...
        payload.append("serviceId", fServiceId);
      } else {
        const picked = reviewableProducts.find((x) => `${x.orderId}-${x.productId}` === fProduct);
        if (!picked) throw new Error("Vui lòng chọn sản phẩm đã mua.");
        payload.append("productId", picked.productId);
        payload.append("productOrderId", picked.orderId);
        payload.append("productOrderDetailId", picked.orderDetailId);
      }
...
setModalOpen(false);
  ...
  function onFileChange(files?: FileList | null) {
    ...
    const incomingKey = new Set(arr.map((file) => `${file.name}-${file.size}-${file.lastModified}`));
    const keptFiles: File[] = [];
    const keptPreviews: string[] = [];

    selectedFiles.forEach((file, index) => {
      const key = `${file.name}-${file.size}-${file.lastModified}`;
      if (incomingKey.has(key)) return;
      keptFiles.push(file);
      keptPreviews.push(imgPreviews[index]);
    });

    const nextFiles = [...keptFiles, ...arr].slice(0, 10);
    const nextPreviews = [...keptPreviews, ...arr.map((file) => URL.createObjectURL(file))].slice(0, 10);

    imgPreviews.forEach((url) => {
      if (!keptPreviews.includes(url)) URL.revokeObjectURL(url);
    });
  ...
  async function fetchHelpfulHistory(page: number) {
    if (!isLoggedIn) return;
    try {
      const res = await http.get<HelpfulHistoryResponse>(`/reviews/my-helpful-reviews?page=${page}&pageSize=${helpfulPageSize}`);
      const payload = res.data;
      setHelpfulHistory((payload.items || []).map(mapPublicReview));
      setHelpfulTotal(payload.total || 0);
      setHelpfulPage(payload.page || 1);
    } catch (e) {
      console.error("Failed to load helpful history", e);
      setHelpfulHistory([]);
      setHelpfulTotal(0);
    }
  }

  function openHelpfulModal() {
    if (!isLoggedIn) {
      redirectToLogin();
      return;
    }
    setHelpfulModalOpen(true);
    fetchHelpfulHistory(1);
  }
      ...
      <AppShell>
          ...
          <Container className="py-16">
            <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
              ...
              <div className="flex gap-2 flex-wrap">
                ...
                <Button variant="ghost" onClick={openHelpfulModal}>
                  Đã đánh dấu hữu ích
                </Button>
              </div>
            </div>
                          ...
                          <div className="p-6 flex flex-col flex-1">
                            <div className="flex-1">
                              <div className="flex items-start justify-between gap-3">
                                <div>
                                  {r.itemPath ? (
                                    <Link to={r.itemPath} className="text-lg font-extrabold text-indigo-700 hover:underline">{r.item}</Link>
                                  ) : (
                              ...
                              <p className="mt-1 text-slate-700 leading-relaxed whitespace-pre-line card p-4 tranform transform !translate-y-0">{r.text}</p>
                            </div>
        ...
        <div className="fixed left-0 right-0 bottom-0 z-40 bg-white/90 backdrop-blur border-t border-slate-200 p-3 md:hidden">
          <Container className="px-2 flex items-center gap-2">
            ...
            <Button variant="ghost" className="flex-1" onClick={openHelpfulModal}>
              Lịch sử hữu ích
            </Button>
          </Container>
        </div>
                            ...
                            .map((r) => (
                                <div key={r!.id} className="rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-200">
                                  <div className="flex items-start justify-between gap-2">
                                    <div>
                                      {r?.itemPath ? (
                                      <Link to={r.itemPath} className="font-extrabold text-indigo-700 hover:underline">{r.item}</Link>
                                    ) : (
        ...
        {helpfulModalOpen && (
          <>
            <div className="fixed inset-0 bg-black/55 z-40" onClick={() => setHelpfulModalOpen(false)} />
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
              <Card className="w-[min(900px,96vw)] max-h-[88vh] overflow-auto p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <SubTitle>History</SubTitle>
                    <div className="text-xl font-extrabold">Đánh giá đã đánh dấu hữu ích</div>
                  </div>
                  <Button variant="ghost" onClick={() => setHelpfulModalOpen(false)}>Đóng</Button>
                </div>
                <Hr className="my-4" />
                {!helpfulHistory.length ? (
                  <Muted>Chưa có đánh giá nào được đánh dấu hữu ích.</Muted>
                ) : (
                  <div className="grid gap-3">
                    {helpfulHistory.map((r) => (
                      <Card key={r.id} className="p-4">
                        <div className="font-extrabold">{r.item}</div>
                        <div className="text-sm text-slate-600 mt-1">{formatDateVi(r.createdAt)} • {r.rating}★</div>
                        <p className="text-sm text-slate-700 mt-2 line-clamp-3 whitespace-pre-line">{r.text}</p>
                        <div className="mt-2 text-xs text-indigo-700 font-bold">{cmsData.reviewCard.helpful}: {r.helpful || 0}</div>
                      </Card>
                    ))}
                  </div>
                )}
                <div className="mt-4 flex items-center justify-end gap-2">
                  <Button variant="ghost" disabled={helpfulPage <= 1} onClick={() => fetchHelpfulHistory(helpfulPage - 1)}>Trước</Button>
                  <span className="text-sm text-slate-600">Trang {helpfulPage} / {Math.max(1, Math.ceil(helpfulTotal / helpfulPageSize))}</span>
                  <Button variant="ghost" disabled={helpfulPage >= Math.ceil(helpfulTotal / helpfulPageSize || 1)} onClick={() => fetchHelpfulHistory(helpfulPage + 1)}>Sau</Button>
                </div>
              </Card>
            </div>
          </>
              ...
              <div className="w-full max-w-3xl my-0" onClick={(e) => e.stopPropagation()}>
                <Card className="overflow-hidden flex flex-col max-h-[calc(100vh-90px)]">
                  ...
                  <div className="p-6 overflow-y-auto flex-1">
                    <div className="grid gap-4">
                      ...
                      <div className="grid md:grid-cols-2 gap-3">
                        <div>
                          ...
                          <div className="mt-2">
                            {fCat === "service" ? (
                              <Select value={fServiceId} onChange={setFServiceId}>
                                <option value="">Chọn dịch vụ</option>
                                {services.map((s) => <option key={s.id} value={String(s.id)}>{s.name}</option>)}
                              </Select>
                            ) : (
                              <Select value={fProduct} onChange={setFProduct}>
                                <option value="">Chọn sản phẩm đã mua</option>
                                {reviewableProducts.map((p) => (
                                  <option key={`${p.orderId}-${p.productId}`} value={`${p.orderId}-${p.productId}`} disabled={p.reviewed}>
                                    {p.productName} - {p.orderCode} {p.reviewed ? "(reviewed)" : ""}
                                  </option>
                                ))}
                              </Select>
                            )}
                        ...
                        <div>
                          ...
                          <div className="mt-2">
                            <Select value={fBranchId} onChange={setFBranchId}>
                      ...
                      <div className="grid md:grid-cols-2 gap-3 items-start">
                        <div>
                          ...
                          <input
                              ...
                              onChange={(e) => onFileChange(e.target.files)} multiple
                          />
                          <div className="text-xs text-slate-500 mt-2">{cmsData.modal.fields.imageHint} (tối đa 10 ảnh)</div>
                          {selectedFiles.length > 0 && (
                            <div className="mt-2 flex items-center justify-between text-xs text-slate-600">
                              <span>Đã chọn {selectedFiles.length} ảnh</span>
                              <button type="button" className="font-bold text-rose-600" onClick={clearSelectedImages}>
                                Xoá tất cả
                              </button>
                            </div>
                          )}
                        ...
                        <div className="rounded-2xl bg-white p-4 ring-1 ring-slate-200">
                          ...
                          <div className="mt-3 overflow-hidden rounded-2xl ring-1 ring-slate-200">
                            {imgPreviews.length ? (
                              <div className="grid grid-cols-2 gap-2 p-2">
                                {imgPreviews.map((src, idx) => (
                                  <div key={src} className="relative">
                                    <img alt={`preview-${idx + 1}`} className="w-full h-24 object-cover rounded" src={src} />
                                    <button
                                      type="button"
                              ...
                              <div className="p-4 text-sm text-slate-500">Chưa có ảnh được chọn</div>
                            )}
                          </div>
                        </div>
                      </div>
                    ...
                    */}
                        <Button tone="brand" variant="solid" className="flex-1" onClick={submitReview} disabled={submitting}>
                          {submitting ? "Đang gửi..." : cmsData.modal.submit}
                        </Button>
                      </div>
                    </div>
                  </div>
