...
export class BlogService implements OnModuleInit, OnModuleDestroy {
  ...
  async listPublic(query: BlogQueryDto) {
    ...
    const filteredItems = query.tag
      ? items.filter((item) => Array.isArray(item.tags) && (item.tags as string[]).includes(query.tag!))
      : items

    return { page, pageSize, total: query.tag ? filteredItems.length : total, items: filteredItems }
  }
