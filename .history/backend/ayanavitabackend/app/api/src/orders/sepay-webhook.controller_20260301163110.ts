...
/** ✅ Narrow union type cho nhánh "emit event" */
function isPaidEmitResult(
  result: unknown,
): result is { ok: true; ignored?: false; orderId: string | number; paymentId: string | number } {
  if (!result || typeof result !== 'object') return false

  const r = result as any
  return (
    r.ok === true &&
    r.ignored !== true && // nếu không có ignored => vẫn OK; nếu ignored=true => không emit
    (typeof r.orderId === 'string' || typeof r.orderId === 'number') &&
    r.orderId !== '' &&
    (typeof r.paymentId === 'string' || typeof r.paymentId === 'number') &&
    r.paymentId !== ''
  )
}