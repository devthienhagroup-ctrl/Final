  ...
  async create(userId: number, dto: CreateProductOrderDto) {
    ...
    const order = await this.prisma.$transaction(async (tx) => {
      ...
      if (!isCod && paymentCode) {
        await tx.productOrderPayment.create({
          data: {
            orderId: created.id,
            provider: 'SEPAY',
            ...
            rawResponse: {
              accountName: 'LÊ MINH HIẾU',
              accountNumber: '4450996789',
              bankName: 'MB BANK',
              bankCode: 'MB',
      ...
      await tx.cartDetail.deleteMany({
        where: {
          ...
          productId: {
            in: pricing.details.map((detail) => detail.productId),
          },
        },
      })
    ...
    const sepayInfo = !isCod
      ? {
          ...
          qrUrl: `https://qr.sepay.vn/img?acc=4450996789&bank=MB&des=${order.paymentCode}&amount=${Number(order.total)}`,
        }
      : null
