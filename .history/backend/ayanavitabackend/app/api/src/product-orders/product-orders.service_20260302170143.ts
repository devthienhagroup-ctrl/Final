...
@Injectable()
export class ProductOrdersService implements OnModuleInit, OnModuleDestroy {
  constructor(private readonly prisma: PrismaService) {}
  private readonly logger = new Logger(ProductOrdersService.name)
  private expirePendingOrdersTimer?: NodeJS.Timeout

  onModuleInit() {
    this.expirePendingOrdersTimer = setInterval(() => {
      void this.expireOverduePendingPaymentOrders()
    }, 60 * 1000)
    void this.expireOverduePendingPaymentOrders()
  }

  onModuleDestroy() {
    if (this.expirePendingOrdersTimer) {
      clearInterval(this.expirePendingOrdersTimer)
    }
  }
  ...
  async create(userId: number, dto: CreateProductOrderDto) {
    ...
    const status = isCod ? ProductOrderStatus.PENDING : ProductOrderStatus.PENDING_PAYMENT
  ...
  async myOrderPaymentQr(userId: number, orderId: number) {
    const order = await this.prisma.productOrder.findFirst({
      where: { id: BigInt(orderId), userId },
      select: {
        id: true,
    ...
    if (order.paymentMethod !== ProductPaymentMethod.SEPAY) {
      throw new BadRequestException('Order does not support QR payment')
    }
    ...
    if (!transferContent) {
      throw new BadRequestException('Order payment content is unavailable')
    }
    ...
    return normalizeBigInt({
      ...
      bankCode: this.bankInfo.gateway,
      transferContent,
      qrUrl: `https://qr.sepay.vn/img?acc=${this.bankInfo.accountNumber}&bank=${this.bankInfo.gateway}&des=${transferContent}&amount=${Number(order.total)}`,
      pollUrl: `/api/product-orders/${order.id}`,
      expiresAt: order.expiresAt,
    })
  }

  async expireOverduePendingPaymentOrders() {
    const now = new Date()

    const result = await this.prisma.$transaction(async (tx) => {
      const updatedOrders = await tx.productOrder.updateMany({
        where: {
          status: ProductOrderStatus.PENDING_PAYMENT,
          expiresAt: { lte: now },
        },
        data: {
          status: ProductOrderStatus.EXPIRED,
        },
      })

      const updatedPayments = await tx.productOrderPayment.updateMany({
        where: {
          order: {
            status: ProductOrderStatus.EXPIRED,
            expiresAt: { lte: now },
        ...
        data: {
          status: ProductPaymentStatus.FAILED,
        },
      })

      return {
        expiredOrders: updatedOrders.count,
        expiredPayments: updatedPayments.count,
      }
    })

    if (result.expiredOrders > 0) {
      this.logger.log(
        `Auto expired ${result.expiredOrders} pending payment orders past 15-minute deadline (payments updated=${result.expiredPayments})`,
      )
    }
