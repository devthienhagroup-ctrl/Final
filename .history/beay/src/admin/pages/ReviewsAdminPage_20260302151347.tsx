...
function mapApiReview(row: ApiReview): Review {
  ...
  return {
    ...
    title: Number(row.stars || 0) >= 4 ? "Rất hài lòng" : Number(row.stars || 0) <= 2 ? "Không như kỳ vọng" : "Tạm ổn",
    content: row.comment || "",
    images: (row.images || []).map((img, idx) => ({ id: String(img.id || `${row.id}-${idx}`), url: img.imageUrl || "" })),
    customer: {
    ...
    flag: "none",
    helpfulUp: Number(row.helpfulCount ?? row._count?.helpfulVotes ?? 0),
    helpfulDown: 0,
    staffNote: "",
...
export function ReviewsAdminPage() {
  ...
  const stats = useMemo(() => {
    ...
    const helpfulTotal = filtered.reduce((sum, r) => sum + r.helpfulUp, 0);
    const helpfulAvg = total ? helpfulTotal / total : 0;

    const dist = [1, 2, 3, 4, 5].map((k) => filtered.filter((r) => r.rating === k).length);

    return { total, visibleCount, hiddenCount, spamCount, withImages, anonymous, avg, dist, helpfulTotal, helpfulAvg };
  }, [filtered]);
  ...
  async function toggleVisibility(id: string) {
    ...
    const isShow = rv.visibility !== "visible";

    try {
      if (isShow) {
        // Backend: mở lại hiển thị
        await api(`/reviews/admin/${Number(id)}/show`, { method: "PATCH" });
        setReviews((prev) => prev.map((r) => (r.id === id ? { ...r, visibility: "visible" } : r)));
        toast("Đã hiện đánh giá.");
      } else {
        // Backend: ẩn
      ...
      const fallbackMsg = isShow ? "Hiện đánh giá thất bại" : "Ẩn đánh giá thất bại";
      toast(e?.message || fallbackMsg);
    }
  }
  ...
  function bulkAction(kind: "show" | "hide" | "spam" | "unspam" | "delete") {
    ...
    openConfirm(
      ...
      () => {
        // NOTE:
        // - show/hide/delete sẽ gọi backend (nếu endpoint tồn tại)
        // - spam/unspam vẫn demo (tuỳ backend mà bạn có thể map thêm endpoint sau)

        if (kind === "delete") {
          (async () => {
            try {
              await Promise.all(ids.map((id) => api(`/reviews/admin/${Number(id)}`, { method: "DELETE" })));
              ...
              toast(`Đã xóa ${ids.length} đánh giá.`);
            } catch (e: any) {
              toast(e?.message || "Xóa hàng loạt thất bại");
            }
          })();
          return;
        }

        if (kind === "show" || kind === "hide") {
          (async () => {
            const endpoint = kind === "show" ? "show" : "hide";
            try {
              await Promise.all(ids.map((id) => api(`/reviews/admin/${Number(id)}/${endpoint}`, { method: "PATCH" })));
              setReviews((prev) =>
                prev.map((r) => {
                  if (!ids.includes(r.id)) return r;
                  const nextVis: ReviewVisibility = kind === "show" ? "visible" : "hidden";
                  return { ...r, visibility: nextVis, logs: [...r.logs, { at: `${r.createdAt} ${r.createdTime}`, by: "Admin", action: kind === "show" ? "SHOW" : "HIDE" }] };
                }),
              );
              toast(`Đã ${label.toLowerCase()} ${ids.length} đánh giá.`);
            } catch (e: any) {
              toast(e?.message || `Thao tác ${label.toLowerCase()} hàng loạt thất bại`);
            }
          })();
          return;
        }

        // Demo-only: spam / unspam
        setReviews((prev) =>
          prev.map((r) => {
            if (!ids.includes(r.id)) return r;
  ...
  return (
    <div className="ar-reviews">
      <style>{`
        ...
        .ar-stat:nth-child(5) .ar-stat-bg{ background: linear-gradient(135deg, #059669, #22c55e); }
      ...
      <div className="ar-container">
        <div className="ar-page-title">
          ...
          <div className="ar-title-actions">
            <button className="ar-pill" onClick={() => window.history.back()} title="Quay lại trang trước">
        ...
        <div className="ar-grid">
          ...
          {/* <div className="ar-card ar-stat">
            <div className="ar-stat-bg" />
            <p className="ar-label">Lượt đánh giá hữu ích</p>
            <div className="ar-row">
              <p className="ar-value">{stats.helpfulTotal}</p>
              <span className="ar-badge-mini"><span className="ar-spark" /> TB {stats.helpfulAvg.toFixed(1)}/review</span>
            </div>
          </div> */}
              ...
              <table className="ar-mini-table">
                ...
                <tbody>
                  {topBadProducts.map(p => (
                    <tr key={p.key}>
                      <td>
                        <div className="ar-entity ar-bad-img-wrapper">
