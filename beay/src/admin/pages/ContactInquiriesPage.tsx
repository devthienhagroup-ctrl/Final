    ...
    import {
        adminContactInquiryStats,
        adminListContactInquiries,
        adminReplyContactInquiry,
        type ContactInquiry,
        type ContactInquiryStatsResponse,
    ...
    export function ContactInquiriesPage() {
        ...
        const [statsData, setStatsData] = useState<ContactInquiryStatsResponse | null>(null);
        ...
        const loadData = async () => {
            ...
            try {
                const [res, statsRes] = await Promise.all([
                    adminListContactInquiries(token, query),
                    adminContactInquiryStats(token, query),
                ]);
                setItems(res.items);
        ...
        const stats = useMemo(() => {
            if (statsData) return statsData.overview;
            ...
            return { total, newCount, repliedCount, emailCount, noteCount: 0, replyCount: 0 };
        }, [items, statsData]);

        const statusChart = useMemo<ChartBarItem[]>(() => {
            if (statsData) {
                return statsData.statusChart.map((it, idx) => ({
                    ...it,
                    tone: (["amber", "emerald", "blue", "violet"] as const)[idx] || "blue",
                }));
            }
        ...
        const trendChart = useMemo(() => {
            if (statsData) return statsData.trendChart;
        ...
        return (
            <div className="relative grid gap-5 px-4 py-5 md:px-6 xl:px-8">
                ...
                <section className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
                    ...
                    <StatCard title="Tổng lượt reply" value={stats.replyCount} hint="Bao gồm toàn bộ phản hồi" icon="fa-solid fa-envelopes-bulk" />
                </section>
                                        ...
                                        <div className="grid gap-3">
                                            {selected.replies.map((r) => (
                                                <div key={r.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                                                    ...
                                                    <div className="mt-3 whitespace-pre-wrap rounded-xl border border-slate-200 bg-white p-3 text-sm leading-6 text-slate-700">
                                                        {r.content}
                                                    </div>
                                                </div>
                                            ))}
